...
module DRb
  ...
  class DRbTCPSocket
    ...
    # jtd: taken from rails/ActiveSupport/MessageVerifier/secure_compare
    ...
    def accept
      ...
      # jtd: adding auth here
      if @config[:secret] != nil
        #puts "drb checking secret"
        shared_secret = @config[:secret]
        @config.delete(:secret)

        received_secret = s.recv(shared_secret.length)
        #puts "received secret: " + received_secret
        if received_secret.length != shared_secret.length
          puts "secret failed length check"
          s.close
        elsif !secure_compare(shared_secret, received_secret)
          puts "secret does not match"
          s.close
        end
        #puts "secret matches!"
      end
  ...
  class DRbServer
    ...
    def initialize(uri=nil, front=nil, config_or_acl=nil)
      android.util.Log::e("NCC", "jtd: DRbServer.initialize")
    ...
    def main_loop
      Thread.start(@protocol.accept) do |client|
        ...
        DRb.mutex.synchronize do
          ...
          #puts "client_uri: " + client_uri.to_s
          #puts "@exported_uri: " + @exported_uri.to_s
  ...
  def start_service(uri=nil, front=nil, config=nil)
    #puts "jtd: starting service with " + uri
    if config == nil
      config = {}
    end
